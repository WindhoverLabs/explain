...
def die_info_rec(die, CU, symbol, struct=None):
    ...
    if die.get_full_path() == symbol:
        try:
            struct_die = CU.get_DIE_at_offset(
                die.attributes['DW_AT_type'].value)
        except KeyError as e:
            print('KeyError, ensure that the structure does not have a tag, '
                  'key: ' + str(e) + '\nExiting...')
            sys.exit(1)
...
def die_member_rec(die, CU, struct):
    ...
    if bit_length == 0:
        bit_length = get_bit_length_from_at_type(
            CU.get_DIE_at_offset(die.attributes['DW_AT_type'].value), CU)
...
def get_bit_length_from_at_type(die, CU):
    ...
    if die.tag == 'DW_TAG_array_type':
        ...
        bit_length = get_bit_length_from_at_type(
            CU.get_DIE_at_offset(die.attributes['DW_AT_type'].value),
            CU) * upper_bound
    else:
        byte_size_attrib = die.attributes.get('DW_AT_byte_size', None)
...
def parse_array_type_child(die, CU):
    ...
    if upper_bound_attrib is not None:
        # Array size is calculated as
        #   (DW_AT_upper_bound - DW_AT_lower_bound) + 1.
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate bitmap files directly from as-built ELF files '
                    'for the libextract library.')
    parser.add_argument('-o', '--output', metavar='<file name>', type=str,
                        help='Output file name', required=True)
    parser.add_argument('-m', '--msgid', metavar='<number>', type=str,
                        help='Message ID.  0x0000 to 0x1fff', required=True)
    parser.add_argument('-ss', '--src-symbol', metavar='<structure name>',
                        type=str, help='Source symbol name.', required=True)
    parser.add_argument('-ds', '--dst-symbol', metavar='<structure name>',
                        type=str, help='Destination symbol name', required=True)
    parser.add_argument('-sf', '--src-file', metavar='<file name>', type=str,
                        help='Source file', required=True)
    parser.add_argument('-df', '--dst-file', metavar='<file name>', type=str,
                        help='Destination file', required=True)
    parser.add_argument('-a', '--append', action='store_true',
                        help='Append to output file.', required=False)
    ...
    if source_struct is None or dest_struct is None:
        print('Error, source or destination structure not found. Ensure the '
              'structure is used, that the compiler has not optimized it out, '
              'and that a tag is not used.\nExiting...')
        sys.exit(1)
